<%@ page language="java" contentType="text/html; charset=UTF-8"
   pageEncoding="UTF-8"%>

<!-- Required meta tags -->
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">


<div class="sect-service">
            <h2>서비스 메뉴</h2>
                <ul class="util">             
                
                    <li><a href="./member/login" class="login"><span>로그인</span></a></li>
               <li><a href="./member/join" class="join"><span>회원가입</span></a></li>
               
               
            </ul>
           </div>


<h1>
   <a href="./"><img
      src="https://img.cgv.co.kr/R2014/images/title/h1_cgv.png" alt="CGV"></a>
</h1>

<div class="im-wrap"> <!-- Important wrap -->
            <h2><img src="https://img.cgv.co.kr/R2014/images/title/h2_cultureplex.png" alt="CULTUREPLEX"></h2>

            <div class="lnb">
               <h2>CGV 주메뉴</h2>
               <ul id="gnb_list">
                  <li class="movie"><a href="/movies/">영화</a>
                     
                  </li>
                  <li class="booking"><a href="/ticket/">예매</a>
                     
                  </li>
                  <li class="theaters"><a href="/theaters/">극장</a>
                     
                  </li>
                  <li class="customer"><a href="/s5/notice/list">고객 센터</a>
                     
                  </li>
               </ul>
            </div>               
            
         </div>